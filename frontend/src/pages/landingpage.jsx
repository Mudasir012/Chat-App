    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)] flex flex-col relative overflow-x-hidden selection:bg-[var(--accent)] selection:text-[var(--accent-content)]">
        ...
        <div className='container max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-16 items-center'>
          ...
          >
            <motion.div 
              ...
              className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[var(--accent)]/5 border border-[var(--accent)]/20 text-[11px] font-semibold text-[var(--accent)] uppercase tracking-wider"
            >
            ...
            <motion.p
              ...
              className="text-lg text-[var(--text-muted)] max-w-lg mx-auto lg:mx-0 leading-relaxed font-light"
            >
            ...
            >
              <Link to="/signup" className="btn-primary flex items-center gap-2 group">
                ...
                <Rocket className="size-4 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-300" />
              </Link>
            ...
            <motion.div
              ...
              className="flex items-center gap-6 justify-center lg:justify-start pt-8 border-t border-[var(--border)]"
            >
              <div className="flex -space-x-2.5">
                {[1, 2, 3, 4].map((i) => (
                  <motion.div
                    ...
                    className="size-9 rounded-full border-2 border-[var(--bg)] bg-[var(--surface)] overflow-hidden flex items-center justify-center text-[10px] font-bold text-[var(--text-muted)]"
                  >
                    {['SJ', 'MK', 'AZ', 'RB'][i-1]}
                  </motion.div>
                ))}
          ...
          >
             ...
             >
               <div className="size-8 rounded-lg bg-[var(--green-bg)] flex items-center justify-center text-[var(--green)] text-sm">
                 🔒
               </div>
               <div>
             ...
             >
               <div className="size-8 rounded-lg bg-[var(--amber-bg)] flex items-center justify-center text-[var(--amber)] text-sm">
                 ⚡
               </div>
               <div>
             ...
             <div className="max-w-[320px] mx-auto p-1 bg-[var(--secondary-bg)]/50 backdrop-blur-2xl rounded-[2rem] border border-[var(--border)] shadow-2xl shadow-black/50 transition-all duration-500 hover:shadow-[0_0_60px_rgba(59,130,246,0.08)]">
      ...
      <section className="w-full flex justify-center py-24 px-4 relative z-10 border-t border-[var(--border)]">
        <div className="container max-w-6xl">
          ...
          >
            {[
              { icon: "🔐", title: "End-to-End encryption", desc: "Every message, file, and call is end-to-end encrypted. Not opt-in — always on.", wide: true },
              { icon: "⚡", title: "Actually instant", desc: "Sub-50ms delivery on our global edge network. No lag, no buffering." },
              { icon: "✨", title: "UI that slaps", desc: "Designed from scratch to be genuinely beautiful. Dark mode that's actually dark." },
              { icon: "🌍", title: "Global by default", desc: "Servers across 40+ regions mean your messages travel the shortest path." },
              { icon: "❤️", title: "Free forever", desc: "Core features stay free. No ads, no data selling, no sketchy monetization." },
              { icon: "🚀", title: "Groups that work", desc: "Up to 500 people, threaded replies, reactions. Group chats that don't devolve." },
            ].map((f, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
                ...
                whileHover={{ y: -6, borderColor: 'var(--accent)', boxShadow: '0 20px 40px -12px rgba(59,130,246,0.15)' }}
                className={`p-8 bg-[var(--secondary-bg)] rounded-[2rem] border border-[var(--border)] transition-all duration-300 cursor-default ${f.wide ? 'md:col-span-2' : ''}`}
              >
                <div className="size-14 rounded-2xl flex items-center justify-center text-2xl mb-8 bg-[var(--accent)]/5">
                  {f.icon}
                </div>
      ...
      <section className="w-full flex justify-center py-24 px-4 relative z-10 border-t border-[var(--border)]">
        <div className="container max-w-6xl">
          ...
          >
            <motion.div
              whileHover={{ y: -4 }}
              className="p-12 bg-[var(--secondary-bg)] rounded-[2.5rem] border border-[var(--border)] flex flex-col justify-between shadow-xl transition-all duration-300"
            >
                ...
                <ul className="space-y-4">
                  {["Unlimited messages", "100MB file storage", "Group chats up to 50", "E2E encryption"].map((item, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm font-light">
                      <div className="size-5 rounded-full bg-[var(--green-bg)] flex items-center justify-center text-[var(--green)]">
            ...
            <motion.div
              whileHover={{ y: -4 }}
              className="p-12 bg-[var(--surface)] rounded-[2.5rem] border border-[var(--accent)]/20 flex flex-col justify-between shadow-2xl relative overflow-hidden transition-all duration-300"
            >
              <div className="absolute top-0 right-0 p-6">
                 <div className="px-4 py-1.5 bg-[var(--accent)] text-[var(--accent-content)] rounded-full text-[10px] font-bold uppercase tracking-widest shadow-lg">Most Popular ✦</div>
              </div>
              <div className="space-y-8 relative z-10">
                ...
                <ul className="space-y-4">
                  {["Everything in Basic", "10GB file storage", "Groups up to 500", "Custom themes", "Priority support"].map((item, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm font-light">
                      <div className="size-5 rounded-full bg-[var(--accent)]/20 flex items-center justify-center text-[var(--accent)]">
                        <Zap className="size-3" />
                      </div>
                      {item}
                    </li>
